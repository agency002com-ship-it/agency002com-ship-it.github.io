...
def patch(html: str) -> str:
    if html.count(OLD_LINK) != 1 or html.count(OLD_BOUNCE) != 1 or html.count(OLD_DONE) != 1:
        raise SystemExit("needles not unique; skip keychain rewrite")
    out = html.replace(OLD_LINK, NEW_LINK).replace(OLD_BOUNCE, NEW_BOUNCE).replace(OLD_DONE, NEW_DONE)
